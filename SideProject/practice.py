...
def register_schedule():
    ...
    if my_res == "yes":
        ...
        print(my_res_date, "과", my_res_cont, "으로 등록하겠습니다. 내용이 맞으면 yes, 틀리면 no라고 말씀해주세요")
        textToVoice(str(my_res_date) + "과" + str(my_res_cont) + "으로 등록하겠습니다. 내용이 맞으면 yes, 틀리면 no라고 말씀해주세요")
        ...
        while True:
            ...
            if my_res1 == " " and (now > after_30sec):
                ...
                if my_res2 == "yes" or my_res2 == "예스":
                    ...
                    textToVoice("일정의 내용을 입력해주세요.")
                    my_res_cont = voiceToText()

                    textToVoice(str(my_res_date) + "에" + str(my_res_cont) + "으로 등록하겠습니다. 내용이 맞으면 yes, 틀리면 no라고 말씀해주세요")
                    ...
                    if my_res_yesno == "yes":
                        ...
                        create_event(start_time, end_time, summary, location, description)
                        textToVoice("여행 일정 등록되었습니다.")
                        textToVoice("지피티모드를 종료하고 대기모드로 돌아갑니다. 제가 필요하면 대기모드에서 지피티라고 불러주세요")
                        # except HttpError as error:
                        #     print(error)
                        #     textToVoice("에러가 발생했습니다.. 지피티 모드를 종료합니다.")
                    else:
